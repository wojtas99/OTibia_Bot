...
def walk(wpt_direction, my_x, my_y, my_z, map_x, map_y, map_z) -> None:
    ...
    if wpt_direction != 0:
        if wpt_direction == 1 and (-2 <= y < 0 or (y == 0 == x)) and abs(z) <= 1:  # Walk North
            win32gui.SendMessage(Addresses.game, win32con.WM_KEYDOWN, rParam[0], lParam[0])
            win32gui.SendMessage(Addresses.game, win32con.WM_KEYUP, rParam[0], lParam[0])
            return
        if wpt_direction == 2 and 0 < y <= 2 and abs(z) <= 1:  # Walk South
            win32gui.SendMessage(Addresses.game, win32con.WM_KEYDOWN, rParam[1], lParam[1])
            win32gui.SendMessage(Addresses.game, win32con.WM_KEYUP, rParam[1], lParam[1])
            return
        if wpt_direction == 3 and 0 < x <= 2 and abs(z) <= 1:  # Walk East
            win32gui.SendMessage(Addresses.game, win32con.WM_KEYDOWN, rParam[2], lParam[2])
            win32gui.SendMessage(Addresses.game, win32con.WM_KEYUP, rParam[2], lParam[2])
            return
        if wpt_direction == 4 and -2 <= x < 0 and abs(z) <= 1:  # Walk West
            win32gui.SendMessage(Addresses.game, win32con.WM_KEYDOWN, rParam[3], lParam[3])
            win32gui.SendMessage(Addresses.game, win32con.WM_KEYUP, rParam[3], lParam[3])
            return
    else:
